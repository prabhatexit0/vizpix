...
function snapshotLayers(layers: EditorState['layers']): LayerSnapshot[] {
  ...
  return layers.map(({ imageBitmap, ...rest }) => ({
    ...rest,
    transform: { ...rest.transform },
  }))
}

async function restoreLayers(snapshots: LayerSnapshot[]): Promise<EditorState['layers']> {
  try {
...
export const createHistorySlice: StateCreator<EditorState, [], [], HistorySlice> = (set, get) => ({
  ...
  undo: async () => {
    ...
    set((s) => ({
      ...
      layers: restored,
      activeLayerId: restored.find((l) => l.id === s.activeLayerId)
        ? s.activeLayerId
