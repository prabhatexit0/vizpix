  ...
  return (
    <Tabs value={activePanel} onValueChange={setActivePanel} className="flex h-full flex-col min-w-0">
      <TabsList className="grid w-full grid-cols-3 shrink-0 rounded-none border-b border-white/15 bg-transparent">
        ...
        <TabsTrigger value="properties" className="text-xs data-[state=active]:bg-white/10 data-[state=active]:text-white">
          Props
        </TabsTrigger>
...
function ResizeHandle({ onResize }: { onResize: (deltaX: number) => void }) {
  ...
  return (
    <div
      className="absolute left-0 top-0 z-10 h-full w-1.5 cursor-col-resize select-none hover:bg-blue-500/40 active:bg-blue-500/60 transition-colors"
    ...
    return (
      <Drawer open={drawerOpen} onOpenChange={setDrawerOpen}>
        <DrawerTrigger asChild>
          <button className="absolute right-4 top-4 z-50 rounded-lg border border-white/10 bg-neutral-900/90 p-2 text-neutral-400 backdrop-blur-md hover:text-white">
            <PanelRight size={18} />
          </button>
        </DrawerTrigger>
