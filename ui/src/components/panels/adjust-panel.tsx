...
export function AdjustPanel() {
  ...
  return (
    <div className="flex flex-col gap-4 p-3">
      {(['brightness', 'contrast', 'saturation'] as const).map((key) => (
        <div key={key}>
          <div className="mb-1.5 flex items-center justify-between">
            <label className="text-xs tracking-wide text-neutral-500 uppercase">{key}</label>
            <span className="text-xs text-neutral-400 tabular-nums">{values[key]}</span>
          </div>
          <Slider
            value={[values[key]]}
            min={-100}
      ...
      <div>
        <div className="mb-1.5 flex items-center justify-between">
          <label className="text-xs tracking-wide text-neutral-500 uppercase">blur radius</label>
          <span className="text-xs text-neutral-400 tabular-nums">{blurRadius}</span>
        </div>
        <Slider
          value={[blurRadius]}
          min={0}
      ...
      <div>
        <div className="mb-1.5 flex items-center justify-between">
          <label className="text-xs tracking-wide text-neutral-500 uppercase">sharpen amount</label>
          <span className="text-xs text-neutral-400 tabular-nums">{sharpenAmount.toFixed(1)}</span>
        </div>
        <Slider
          value={[sharpenAmount]}
          min={0}
      ...
      <div>
        <div className="mb-1.5 flex items-center justify-between">
          <label className="text-xs tracking-wide text-neutral-500 uppercase">sharpen radius</label>
          <span className="text-xs text-neutral-400 tabular-nums">{sharpenRadius}</span>
        </div>
        <Slider
          value={[sharpenRadius]}
          min={1}
      ...
      <div>
        <div className="mb-1.5 flex items-center justify-between">
          <label className="text-xs tracking-wide text-neutral-500 uppercase">threshold</label>
          <span className="text-xs text-neutral-400 tabular-nums">{sharpenThreshold}</span>
        </div>
        <Slider
      ...
      <div>
        <div className="mb-1.5 flex items-center justify-between">
          ...
          <span className="text-xs text-neutral-400 tabular-nums">{numColors}</span>
        </div>
        <Slider
          value={[numColors]}
          min={2}
